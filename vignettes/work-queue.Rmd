---
title: "Implementing a Work Queue using rpsql"
author: "Jamie Lentin"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Implementing a Work Queue using rpsql}
...
n <- NULL
while (is.null(n)) {
  n <- rpsql::postgresWaitForNotify(con)
}
n$payload
```
...
worker <- function() {
  library(DBI)
  db_worker <- dbConnect(rpsql::Postgres())
  ...
  while (TRUE) {
    ...
    n <- rpsql::postgresWaitForNotify(db_worker, 60)
    if (is.null(n)) {
